...
function crear_promocion()
{
	...
	if (empty($_POST['diasSemana']) || !is_array($_POST['diasSemana'])) {
	...
	if (!empty($errores)) {
		...
		// Cuando migren la vista del dueño, esto será index.php?vista=darAltaPromos
	...
	if ($stmt->execute()) {
		$_SESSION['success'] = "Promoción agregada exitosamente. El Administrador debe evaluarla.";
...
function aprobar_promocion_admin()
{
	...
	$stmt->close();
	header("Location: ../../index.php?vista=admin_promociones"); // Ruta fixeada
	exit();
}
